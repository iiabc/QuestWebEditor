...
interface EntityInterpreterFieldProps {
    value: string;
...
// API 配置：每个 API 支持的属性
const ENTITY_API_CONFIG: Record<string, { label: string; properties: Array<{ value: string; label: string; type: 'string' | 'number' }> }> = {
    minecraft: {
        label: 'Minecraft',
        properties: [
            { value: 'type', label: 'Type (类型)', type: 'string' },
    ...
    MythicMobs: {
        label: 'MythicMobs',
        properties: [
            { value: 'id', label: 'ID (怪物标识符)', type: 'string' },
            { value: 'type', label: 'Type (怪物类型)', type: 'string' },
            { value: 'level', label: 'Level (等级，大于该值)', type: 'number' },
            { value: 'faction', label: 'Faction (阵容)', type: 'string' },
            { value: 'health', label: 'Health (最大血量)', type: 'number' },
            { value: 'damage', label: 'Damage (伤害)', type: 'number' },
        ]
    },
};

// API 选项列表
const ENTITY_API_OPTIONS = Object.keys(ENTITY_API_CONFIG).map(key => ({
    value: key,
    label: ENTITY_API_CONFIG[key].label
}));

export const EntityInterpreterField: React.FC<EntityInterpreterFieldProps> = ({ value, onChange }) => {
    ...
    const availableProperties = useMemo(() => {
        return ENTITY_API_CONFIG[namespace]?.properties || [];
    }, [namespace]);
    ...
    useEffect(() => {
        ...
        if (parts.length > 0) {
            ...
            const validNamespace = ENTITY_API_CONFIG[parsedNamespace] ? parsedNamespace : 'minecraft';
            ...
            if (strVal.includes(':')) {
                ...
                // 检查 namespace 是否在配置中
                const validNamespace = ENTITY_API_CONFIG[ns] ? ns : 'minecraft';
                setNamespace(validNamespace);
                if (rest.length > 0) {
                    const firstProp = ENTITY_API_CONFIG[validNamespace]?.properties[0]?.value || 'type';
                    setProperties([{ key: firstProp, value: rest.join(':') }]);
                }
            } else {
                // 默认使用 minecraft
                setNamespace('minecraft');
                const firstProp = ENTITY_API_CONFIG['minecraft']?.properties[0]?.value || 'type';
    ...
    return (
        <Stack gap="xs">
            <Group grow>
                <Select
                    data={ENTITY_API_OPTIONS}
                    value={namespace}
                    onChange={(val) => {
                        if (val) {
                            setNamespace(val);
                            // 切换 API 时，清空不兼容的属性
                            const newApiConfig = ENTITY_API_CONFIG[val];
                            if (newApiConfig) {
